...
import { LayoutDashboard, FileText, Users, Tags, BarChart3, Settings, X, LogOut, type LucideIcon } from 'lucide-react'
...
export default function AdminSidebar({ open = false, onClose }: AdminSidebarProps) {
  // Guaranteed authenticated with a resolved role — this layout only renders under RequireAuth + RequireRole.
  ...
  return (
    <>
      {open && (
        <button aria-label="Close menu" onClick={onClose} className="fixed inset-0 z-40 bg-ink/30 lg:hidden" />
      )}
      <aside
        className={cn(
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  ...
                  'focus-ring',
                  isActive ? 'bg-navy-tint text-navy' : 'text-ink-muted hover:bg-paper hover:text-ink'
                )
              }
            >
