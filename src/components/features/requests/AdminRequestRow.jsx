...
function numericId(displayId) {
  return Number(displayId.replace('REQ-', ''))
}

export default function AdminRequestRow({ request, onUpdated }) {
  ...
  async function handleStatusChange(event) {
    ...
    setError(null)
    try {
      const data = await api.patch(`/admin/requests/${numericId(request.id)}/status`, { status })
