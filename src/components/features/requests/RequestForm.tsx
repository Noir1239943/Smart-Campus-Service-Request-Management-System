...
import type { CategoryItem, FieldErrors } from '@/types'
...
export default function RequestForm({ offices, requestTypes, onSubmitted }: RequestFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    body.set('request_type_id', (elements.namedItem('request-type') as HTMLSelectElement).value)
    body.set('office_id', (elements.namedItem('office') as HTMLSelectElement).value)
    body.set('subject', (elements.namedItem('subject') as HTMLInputElement).value)
    body.set('details', (elements.namedItem('details') as HTMLTextAreaElement).value)
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        >
          ...
          <span className="text-sm text-ink-muted">{fileName ?? 'Click to upload, or drag a file here'}</span>
      ...
      <div className="flex items-center justify-between border-t border-border pt-5">
        <p className="text-xs text-ink-faint">You'll get a notification once the office updates your request.</p>
