...
interface CategoryRowProps {
  ...
  onDelete: (id: CategoryItem['id']) => void
}

function CategoryRow({ item, endpoint, onChange, onDelete }: CategoryRowProps) {
  ...
  async function save() {
    ...
    setError(null)
    try {
      const data = await api.patch<unknown>(`${endpoint}/${item.id}`, { name: name.trim() })
...
export default function CategoryManager({ title, description, endpoint, items, onItemsChange }: CategoryManagerProps) {
  ...
  async function addItem(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null)
    try {
      const data = await api.post<unknown>(endpoint, { name: newName.trim() })
      onItemsChange([...items, unwrapItem<CategoryItem>(data)])
      setNewName('')
    } catch (err) {
  ...
  function handleDelete(id: CategoryItem['id']) {
