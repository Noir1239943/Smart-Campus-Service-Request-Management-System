...
import type { AdminDashboardStats, ServiceRequest } from '@/types'
...
export default function AdminDashboardPage() {
  const [stats, setStats] = useState<AdminDashboardStats>({ total: 0, awaiting: 0, completed: 0, total_users: 0 })
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((data) => {
        if (cancelled) return
        setStats(data.stats)
        setRecentRequests(unwrapCollection<ServiceRequest>(data.recent_requests))
      })
